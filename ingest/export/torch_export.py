...
sys.path = [p for p in sys.path if "/nix/store/" not in p or "site-packages" not in p]
...
from tron_ingest_tools import (  # noqa: E402
  ExportableModel,
...
def _get_auto_model_class(config) -> type:
  ...
  for arch in architectures:
    ...
    if "formaskedlm" in arch_lower or "forsequenceclassification" in arch_lower:
...
def _move_to_meta(module: torch.nn.Module) -> None:
  ...
  for name, buffer in list(module.named_buffers(recurse=False)):
    if buffer.device.type != "meta":
      new_buffer = torch.empty(buffer.shape, dtype=buffer.dtype, device="meta")
...
def init_model_no_weights(
  ...
  if use_meta_device:
    ...
    print("Using meta device - zero memory footprint for structure export.")
    return model
  ...
  def materialize_with_zeros(module):
    ...
    for name, buffer in module.named_buffers(recurse=False):
      if buffer.device.type == "meta":
        new_buffer = torch.zeros(buffer.shape, dtype=buffer.dtype, device="cpu")
        module.register_buffer(name, new_buffer)
...
def _build_attention_mask(
  ...
  if sliding_window is not None:
    ...
    attention_masks = transformers.masking_utils.create_masks_for_generate(
      ...
      position_ids=torch.zeros((batch_sz, actual_seq_len), dtype=torch.long),
    )

    if isinstance(attention_masks, dict) and "sliding_attention" in attention_masks:
      full_mask = attention_masks["full_attention"].to(target_device)
      sliding_mask = attention_masks["sliding_attention"].to(target_device)
      metadata = {
        "attention_masks": {
    ...
    metadata = {"attention_masks": {"attention_mask": {"type": "full_causal"}}}
...
def _export_model_impl(
  ...
  if hasattr(config, "text_config") and any(
    "ForConditionalGeneration" in a for a in getattr(config, "architectures", []) or []
  ):
    print(
  ...
  if load_weights:
    ...
    if quant_config is not None and quant_config.get("quant_method") == "mxfp4":
      ...
      print("Using mxfp4 dequantization to get model-format weight shapes")
  ...
  try:
    ...
    if tokenizer_name:
      print(f"Tokenizer not found at {model_name}, downloading from {tokenizer_name}")
      tokenizer = transformers.AutoTokenizer.from_pretrained(
  ...
  try:
    ...
    if "Constraints violated" in err_str and "seq_len" in err_str:
      ...
      print("seq_len specialization detected, retrying with static seq_len")
  ...
  metadata["config"] = {k: getattr(text_config, k, None) for k in CONFIG_KEYS}
  ...
  metadata["tag_config"] = {k: getattr(text_config, k, None) for k in TAG_KEYS}
  ...
  metadata["config"]["rope_theta"] = rope_theta if rope_theta is not None else 0.0
...
def main() -> None:
  ...
  for node in exported.graph.nodes:
    if node.op == "placeholder" and hasattr(node, "meta") and "val" in node.meta:
      val = node.meta["val"]
