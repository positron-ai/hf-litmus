    ...
    for name in self.param_names:
      if name == "input_ids_1":
        ...
        if buf_name in self.hf_buffers:
          kwargs[name] = self.hf_buffers[buf_name].to(device=device, dtype=model_dtype)
...
def load_generated_model(py_file: Path, model_class_name: str) -> torch.nn.Module:
  ...
  if hasattr(module, model_class_name):
    ...
    print(f"Warning: Class {model_class_name} not found, using {candidates[0]}")
...
def smart_reconstruct_hf_name(inner: str) -> str:
  ...
  while i < len(tokens):
    ...
    if tokens[i] == "model":
      result.append("model")
      i += 1
    elif tokens[i] == "layers" and i + 1 < len(tokens) and tokens[i + 1].isdigit():
...
def transfer_weights(
  ...
  for gen_name, gen_param in gen_state.items():
    ...
    if hf_name in hf_state:
      ...
      if hf_weight.shape == gen_param.shape:
        with torch.no_grad():
          gen_param.copy_(hf_weight.to(device=gen_param.device, dtype=gen_param.dtype))
        transferred += 1
      else:
        print(
  ...
  if missing_in_hf:
    print(f"Warning: Missing in HF state dict: {len(missing_in_hf)} parameters")
