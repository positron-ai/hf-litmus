...
def create_parser() -> argparse.ArgumentParser:
  parser = argparse.ArgumentParser(
    description=(
      "HF Litmus: Continuous model compatibility"
      " testing for Tron"
    ),
    formatter_class=(
  ...
  parser.add_argument(
    ...
    default=7200,
    help=(
      "Timeout for deep analysis Claude Code"
      " session in seconds"
    ),
  )
  parser.add_argument(
  ...
  parser.add_argument(
    ...
    default=None,
    help=(
      "Notion page ID under which to create"
      " litmus result pages"
    ),
  )
  parser.add_argument(
  ...
  try:
    ...
    if config.model_file:
      ...
      if not models:
        ...
        return 1
      logging.info(
        "Processing %d models from %s"
        " (jobs=%d)",
        len(models),
        config.model_file,
        config.max_jobs,
      )
      orchestrator.run_model_list(
        models, max_jobs=config.max_jobs
      )
