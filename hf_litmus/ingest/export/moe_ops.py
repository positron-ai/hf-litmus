...
def moe_router_proxy(
  ...
  **body_kwargs,
  ...
  if mode.enable_tracing:
    ...
    all_body_args, sorted_kwarg_keys = _flatten_kwargs(body_args, body_kwargs)
    ...
    all_body_args_proxies = [outer_tracer.unwrap_proxy(arg) for arg in all_body_args]

    # Collect captured tensors from the body graph's placeholders
    # Placeholders beyond explicit args are captures
    placeholders = [n for n in body_graph.graph.nodes if n.op == "placeholder"]
    ...
    return moe_router_op(body_fn, topk_indices, topk_scores, *body_args, **body_kwargs)
...
def moe_router_fake(
  ...
  **body_kwargs,
  ...
  with mode:
    example_out = body_fn(example_idx, example_token_indices, *body_args, **body_kwargs)

  # Output shape: (num_tokens, *body_out_dim) - blended result
  # Body returns (num_selected, *out_dim)
  body_out_dim = example_out.shape[1:]  # everything after num_selected
  out_shape = (num_tokens,) + body_out_dim
  return torch.empty(out_shape, dtype=example_out.dtype, device=example_out.device)
...
def moe_router_eager(
  ...
  **body_kwargs,
  ...
  for expert_idx in unique_experts:
    ...
    expert_out = body_fn(expert_idx, token_indices, *body_args, **body_kwargs)
    ...
    for i, (tok, k) in enumerate(zip(token_indices.tolist(), k_indices.tolist())):
