...
def export_fx_trace(
  ...
  if device.type == "meta":
    input_ids = torch.empty((BATCH_SZ, seq_len), dtype=torch.long, device="meta")
    attention_mask = torch.empty(
      (BATCH_SZ, 1, seq_len, seq_len), dtype=dtype, device="meta"
    )
  else:
    input_ids = torch.zeros((BATCH_SZ, seq_len), dtype=torch.long, device=device)
    ...
    attention_mask = causal_mask.unsqueeze(0).unsqueeze(0).expand(BATCH_SZ, -1, -1, -1)
...
def _is_moe_weight_transposed(target: str, shape: List[int], config: dict) -> bool:
...
def _parse_stack_trace(trace: str) -> list[dict[str, str | int]]:
  ...
  for match in re.finditer(r'File "([^"]+)", line (\d+), in ([\w<>]+)', trace):
  ...
  for node in graph.nodes:
    ...
    if nn_stack:
      entry["nn_module_stack"] = {
        path: qualified_name for path, (qualified_name, _cls) in nn_stack.items()
      }
    ...
    if src_stack:
      if isinstance(src_stack, dict):
        entry["source_fn_stack"] = {
          name: getattr(fn, "__name__", str(fn)) for name, fn in src_stack.items()
        }
      else:
        entry["source_fn_stack"] = {
          name: getattr(fn, "__name__", str(fn)) for name, fn in src_stack
        }
...
def build_ingest_metadata(
  ...
  constants: List[dict] = []
  for spec in sig.input_specs:
    if spec.kind == InputKind.CONSTANT_TENSOR and hasattr(spec.arg, "name"):
        ...
        constants.append(
          {
            ...
            "value": (
              float(const) if len(const.shape) == 0 else list(const.flatten().tolist())
            ),
          }
        )
  ...
  for node in exported.graph.nodes:
    if node.op == "placeholder" and hasattr(node, "meta") and "val" in node.meta:
      ...
      if hasattr(val, "shape") and hasattr(val, "dtype"):
        ...
        input_metadata[node.name] = {"shape": shape, "dtype": str(val.dtype)}
  ...
  range_constraints = {
    k.name: (str(v.lower), str(v.upper)) for k, v in exported.range_constraints.items()
  }
