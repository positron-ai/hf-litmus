...
class NotionPublisher:
    ...
    async def _publish_result_async(
        ...
        async with (
            streamablehttp_client(self.mcp_url) as (
                read_stream,
                write_stream,
                _,
            ),
            ...
            logger.debug("Notion MCP tools: %s", tool_names)
    ...
    async def _find_or_create_database(
        ...
        if self.database_id:
            ...
            logger.warning(
                "Cached database ID %s invalid, searching for database",
                self.database_id,
            )
            self.database_id = ""
    ...
    async def _validate_database(
        ...
        try:
            ...
            return bool(data and data.get("object") == "database")
        except Exception:
            return False
    ...
    async def _create_database(
        ...
        if not create_tool:
            logger.warning(
                "No database creation tool found. Available: %s",
                tool_names,
            )
            return ""
    ...
    async def _create_entry(
        ...
        if not create_tool:
            logger.error(
                "No page creation tool found. Available: %s",
                tool_names,
            )
            return None
        ...
        try:
            ...
            logger.debug(
                "Retrying entry creation with parent_database_id param"
            )
            try:
                resp = await session.call_tool(
                    create_tool,
        ...
        if not page_id:
            logger.error("Could not extract page ID from create response")
            return None
    ...
    async def _update_entry(
        ...
        if update_tool:
            ...
            try:
                ...
                logger.warning(
                    "Property update failed for %s, appending content only",
                    result.model_id,
                    exc_info=True,
                )
        else:
            logger.debug("No update tool, appending content only")

        # Append new run section with divider
        now = datetime.now(timezone.utc)
        divider_blocks = [
            _divider(),
            _heading2(f"Run: {now:%Y-%m-%d %H:%M UTC}"),
        ]
    ...
    def _build_properties(
        ...
        status = result.status.value
        stage = result.failure_stage.value if result.failure_stage else ""
        ...
        return {
            ...
            "Pipeline Tag": _rich_text_prop(result.pipeline_tag),
            ...
            "Ingest Version": _rich_text_prop(result.ingest_version),
    ...
    def _build_blocks(
        ...
        blocks.append(_paragraph(f"**Model:** {result.model_id}"))
        blocks.append(_paragraph(f"**Pipeline Tag:** {result.pipeline_tag}"))
        blocks.append(_paragraph(f"**Downloads:** {result.downloads:,}"))
        blocks.append(_paragraph(f"**Likes:** {result.likes:,}"))
        blocks.append(
            _paragraph(f"**Tested:** {result.tested_at:%Y-%m-%d %H:%M UTC}")
        )
        blocks.append(
            _paragraph(f"**Ingest Version:** {result.ingest_version}")
        )
        ...
        if result.status == ModelStatus.SUCCESS:
            ...
            blocks.append(_paragraph(f"**Status:** {result.status.value}"))
            blocks.append(_paragraph(f"**Failure Stage:** {stage}"))
            blocks.append(_paragraph(f"**Failure Class:** {fclass}"))
        ...
        # Gap data sections
        if gap_data:
            furthest = gap_data.get("furthest_stage", "")
            if furthest:
                blocks.append(_heading2("Pipeline Progress"))
                blocks.append(_paragraph(f"**Furthest Stage:** {furthest}"))
            ...
            kernels = gap_data.get("missing_kernels", [])
            if kernels:
                blocks.append(_heading2("Missing Kernels"))
                for k in kernels:
                    blocks.append(_bulleted(f"`{k}`"))

            patterns = gap_data.get("missing_patterns", [])
            if patterns:
                blocks.append(_heading2("Missing Patterns"))
                for p in patterns:
            ...
            if blockers:
                ...
                for b in blockers:
                    ...
                    blocks.append(
                        _bulleted(
                            f"[{sev}] {bstage}: {desc} (effort: {effort})"
                        )
                    )

            # Include raw JSON
            blocks.append(_heading2("Gap Summary (JSON)"))
            blocks.append(
                _code_block(
        ...
        # Analysis markdown
        if analysis_md:
            blocks.append(_heading2("Detailed Analysis"))
            md_blocks = _markdown_to_blocks(analysis_md)
            blocks.extend(md_blocks)

        # Error output (truncated)
        if result.error_output and result.status != ModelStatus.SUCCESS:
            blocks.append(_heading2("Error Output"))
            error_text = result.error_output[:3000]
            blocks.append(_code_block(error_text, language="plain text"))
    ...
    async def _append_blocks(
        ...
        if not append_tool:
            logger.warning(
                "No block append tool found. Available: %s",
                tool_names,
            )
            return
        ...
        for i in range(0, len(blocks), batch_size):
            ...
            try:
                ...
                logger.warning(
                    "Failed to append blocks batch %d-%d for page %s",
                    i,
                    i + len(batch),
                    page_id,
                    exc_info=True,
    ...
    async def _append_as_markdown(
        ...
        for b in blocks:
            ...
            if btype in b:
                ...
                if rt:
                    text = rt[0].get("text", {}).get("content", "")
            if text:
        ...
        if simple:
            try:
                ...
                logger.warning(
                    "Markdown fallback also failed for page %s",
                    page_id,
                    exc_info=True,
                )
    ...
    for item in resp.content:
        if hasattr(item, "text"):
            ...
            try:
                ...
                if isinstance(data, dict):
                    pid = data.get("id", data.get("page_id", ""))
                    if pid:
                        return str(pid)
            except (json.JSONDecodeError, TypeError):
                pass
