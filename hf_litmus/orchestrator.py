...
class LitmusOrchestrator:
  ...
  def setup_signal_handlers(self) -> None:
    ...
    def handler(signum: int, frame: object) -> None:
      if self._shutdown:
        ...
        raise SystemExit(1)
      logger.info("Shutdown requested, finishing current model... (repeat to force)")
      self._shutdown = True
  ...
  def process_model(
    ...
    with tempfile.TemporaryDirectory(prefix="litmus_trace_") as trace_tmp:
      ...
      # Step 1: Export
      export_result = self.export_runner.export_model(model_id, trace_dir)
      ...
      if not export_result.success:
        ...
        result.status = (
          ModelStatus.TIMEOUT if export_result.timed_out else ModelStatus.EXPORT_FAIL
        )
      ...
      if not ingest_result.success:
        ...
        result.status = (
          ModelStatus.TIMEOUT if ingest_result.timed_out else ModelStatus.INGEST_FAIL
        )
  ...
  def _load_gap_data(
    ...
    gap_path = self.config.output_dir / "analyses" / sanitized / "gap-summary.json"
    if gap_path.exists():
      try:
  ...
  def _publish_to_notion(
    ...
    if self.state.notion_database_id and not self._notion_publisher.database_id:
    ...
    try:
      ...
      if self._notion_publisher.database_id:
        self.state.notion_database_id = self._notion_publisher.database_id
    except Exception:
      logger.warning(
