...
def generate_summary(
  state: StateManager, output_dir: Path
) -> None:
  ...
  summary_data = _compute_summary(
    results, output_dir, gap_summaries
  )
...
def _compute_summary(
  ...
  high_impact = sorted(
    failed, key=lambda r: r.downloads, reverse=True
  )[:20]
  ...
  salient = compute_salient_points(
    results, gap_summaries
  )
  ...
  for r in results:
    if r.status != ModelStatus.SUCCESS:
      origin = (
        r.failure_origin.value
        if r.failure_origin else "unknown"
      )
  ...
  for r in results:
    ...
    entry: dict = {
      ...
      "failure_origin": (
        r.failure_origin.value
        if r.failure_origin else None
      ),
      "retryable": r.retryable,
      "model_tags": r.model_tags,
      "assessment": assessment,
      "missing_ops": r.missing_ops,
      "missing_feature_areas": sorted(
        missing_areas
      ),
    }
...
def _format_markdown(data: dict) -> str:
  ...
  for item in data["high_impact_failures"]:
    blockers = ", ".join(
      f"`{op}`" for op in item["missing_ops"]
    )
  ...
  consensus_models = [
    item for item in data["high_impact_failures"]
    if item.get("assessment", {}).get(
      "confidence"
    ) == "consensus"
  ]
  if consensus_models:
...
def _format_executive_summary(
  ...
  if ops:
    lines.append(
      f"**Missing ops** ({len(ops)} total):"
    )
    for op in ops[:15]:
      lines.append(f"- `{op}`")
    if len(ops) > 15:
      lines.append(
        f"- ... and {len(ops) - 15} more"
      )
    lines.append("")
  ...
  passing = salient.get(
    "observed_passing_architectures", []
  )
  if passing:
    lines.append(
      "**Observed passing architectures:** "
      + ", ".join(passing)
    )
    lines.append("")
