...
def generate_report(
    ...
    frontmatter = {
        ...
        "failure_origin": (
            result.failure_origin.value if result.failure_origin else None
        ),
    ...
    lines = [
        ...
        (f"| Tested At | {result.tested_at:%Y-%m-%d %H:%M:%S UTC} |"),
        f"| Ingest Version | {result.ingest_version} |",
    ]
    if result.model_tags:
        lines.append(f"| Model Tags | {', '.join(result.model_tags)} |")
    if result.failure_origin:
        lines.append(f"| Failure Origin | {result.failure_origin.value} |")
    if result.retryable:
    ...
    if result.status == ModelStatus.SUCCESS:
        ...
        if result.missing_ops:
            ops = ", ".join(f"`{op}`" for op in result.missing_ops)
        ...
        if result.analysis_path:
            ...
            lines.extend(
                [
                    ...
                    "",
                    f"- **Analysis**: [{display}]({url})",
                ]
            )
            if result.analysis_branch:
                lines.append(f"- **Branch**: `{result.analysis_branch}`")
...
def _generate_analysis(result: ModelResult) -> str:
    ...
    return analyses.get(
        fc,
        f"Classified as '{fc}'. Manual investigation may be needed.",
    )
...
def generate_model_metadata(
    ...
    assessment = compute_model_assessment(result, gap_data)
    ...
    metadata: dict = {
        ...
        "retryable": result.retryable,
        "model_tags": result.model_tags,
        "deep_analysis_error": (result.deep_analysis_error or None),
    ...
    if gap_data:
        metadata["gap_analysis"] = {
            "status": "completed",
            "furthest_stage": gap_data.get("furthest_stage", None),
            "missing_kernels": gap_data.get("missing_kernels", []),
            "missing_patterns": gap_data.get("missing_patterns", []),
            "fixes_applied": gap_data.get("fixes_applied", []),
            "consensus_review": gap_data.get("consensus_review", None),
        }
    else:
        metadata["gap_analysis"] = {
            "status": "not_run",
