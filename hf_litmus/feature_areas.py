...
def _classify_prose_verdict(prose: str) -> str:
    ...
    if "far" in lower or "significant" in lower:
        return "far"
    if "close" in lower or "minor" in lower or "pass" in lower:
        return "close"
    return "far"
...
def _assessment_from_gap(
    ...
    efforts = [
        b.get("effort", "medium") for b in blockers if isinstance(b, dict)
    ]
    if any(e == "large" for e in efforts) or len(efforts) > 3:
        effort = "large"
    elif efforts:
        effort = "medium"
    else:
    ...
    # Determine verdict
    if (total_blockers == 0 and not missing_ops) or total_blockers <= 2:
        verdict = "close"
    ...
    if isinstance(consensus, dict):
        ...
        if isinstance(ext, dict):
            ...
            if isinstance(additional, list):
                areas.update(r for r in additional if isinstance(r, str))

    # Validate furthest_stage against enum
    raw_stage = gap_data.get("furthest_stage", "unknown")
    stage = raw_stage if raw_stage in _STAGE_VALUES else "unknown"
...
def _assessment_heuristic(
    ...
    fc = result.failure_class.value if result.failure_class else "unknown"
...
def load_gap_summaries(
    ...
    for gap_path in analyses_dir.glob("*/gap-summary.json"):
        try:
            ...
            logger.warning("Failed to parse %s: %s", gap_path, e)
...
def compute_salient_points(
    ...
    for r in results:
        ...
        elif r.failure_class == FailureClass.UNSUPPORTED_DYNAMIC:
            systemic.add("data-dependent control flow")
        elif r.failure_class == FailureClass.MEMORY_ERROR:
            systemic.add("memory limits")

    # Enrich from gap summaries
    for _model_id, gap in gap_summaries.items():
    ...
    return {
        "missing_feature_areas": [area for area, _ in all_areas.most_common()],
        "missing_ops": [op for op, _ in all_missing.most_common()],
        "systemic_issues": sorted(systemic),
        "observed_passing_architectures": sorted(passing_tags),
        "sources": sources,
    }
